...
public class CategoryEditorDialog extends AbstractEditorDialog<Category> {
    ...
    public CategoryEditorDialog(BiConsumer<Category, Operation> itemSaver) {
        super("category", itemSaver);

        BeanManager beanManager = BeanManagerProvider.getInstance()
                .getBeanManager();
        Bean<?> bean = beanManager
                .resolve(beanManager.getBeans(CategoryService.class));
        categoryService = (CategoryService) beanManager.getReference(bean,
                bean.getBeanClass(), beanManager.createCreationalContext(bean));

        bean = beanManager.resolve(beanManager.getBeans(ReviewService.class));
        reviewService = (ReviewService) beanManager.getReference(bean,
                bean.getBeanClass(), beanManager.createCreationalContext(bean));
    ...
    protected void confirmDelete() {
        int reviewCount = reviewService.findReviews(getCurrentItem().getName())
                .size();
        if (reviewCount > 0) {
