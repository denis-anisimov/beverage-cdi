...
import com.vaadin.starter.beveragebuddy.ui.common.DeleteItemEvent;
...
public class ReviewsList extends PolymerTemplate<ReviewsModel> {
    ...
    private void deleteUpdate(@Observes DeleteItemEvent event) {
        Object item = event.getItem();
        if (item instanceof Review) {
            reviewService.deleteReview((Review) item);
            updateList();
    ...
    private void updateList() {
        List<Review> reviews = reviewService.findReviews(search.getValue());
        if (search.isEmpty()) {
